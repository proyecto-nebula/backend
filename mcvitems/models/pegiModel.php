...
// Clase del modelo para trabajar con la tabla PEGI
class PegiModel
{
    ...
    public function getAll() {
        session_start();
        $curl = curl_init($this->url_apirest. "pegi.php");
        ...
        foreach ($array_datos as $datos) {
            $m = new PegiModel();
            $m->id_pegi = $datos['id_pegi'];
            $m->imagen = $datos['imagen'];
            $models[] = $m;
        }
        return $models;
    }
    ...
    public function delete($id){
        session_start();
        $ch = curl_init($this->url_apirest. "pegi.php?id_pegi=". $id);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
